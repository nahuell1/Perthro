...
- `ollama` para chatear con el modelo deployado en el servidor que corre el bot (está configurado para correr llama3.1 corriendo en ollama, pero la biblioteca utilizada es reemplazable para utilizar OpenAi)
- `hello` para probar el bot. Responde con "Hola {nombre de usuario}".
- `tiempo` para consultar el clima en tiempo real con información obtenida de internet, preparado para consultar la api de `openmeteo`, con respuesta humanizada.
